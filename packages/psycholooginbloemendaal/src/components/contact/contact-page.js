        ...
        <Formik
          ...
          onSubmit={(values, { setSubmitting }) => {
            setTimeout(() => {
              ...
              emailjs.sendForm('service_g86r2il', 'template_i7f32xz', 'form').then(
                function (response) {
                  console.log('Email gestuurd', response.status, response.text);
                },
                function (error) {
        ...
        >
          {({ values, errors, touched, handleChange, handleBlur, handleSubmit, isSubmitting }) => (
            <form id='form' onSubmit={handleSubmit}>
              <input type='text' name='name' onChange={handleChange} value={values.name} placeholder='Naam' />
              ...
              <textarea name='message' onChange={handleChange} value={values.message} placeholder='Vraag' />
